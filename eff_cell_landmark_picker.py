...
class MainWindow():
    ...
    def __init__(self, main):
        ...
        self.button_add_landmark = tk.Button( self.main, text="Add landmarks", fg="black", command=self.add_landmark_toggle )
        self.button_add_landmark.pack()
        
        self.button_landmark_counter = tk.Button(self.main, text="Next landmark", fg="black", command=self.landmark_count)
...
#TODO: make this function for landmarks
    ...
    def export_data(self):
        ...
        with open(filename, "w") as csv_file:
            ...
            for nr in range(n_landmarks):
                ...
                save_str = "{:4.0f}, {:4.0f}, {:1.0f}, {:7.2f}, {:7.2f}".format( self.landmarks[nr][0], self.landmarks[nr][1], self.landmark_ids[nr], im1_int, im2_int)
 ...
 #       for lm in range(self.landmarks):
    ...
    def about(self):
        ...
        msg = tk.Message(top, text="This little gui can display several images, in which the locations of a given neuron can be annotated over multiple imaging sessions, with the purpose of using the annotations to align"
                                   "stacks to each other.", width=600)
        msg.pack()
        msg = tk.Message(top, text="Manual:", width=600)
        msg.pack()
        msg = tk.Message(top, text="- Select images: Select several images, from the same location and channel, from different dates.", width=600)
        msg.pack()
        msg = tk.Message(top, text="- Export: Saves landmarks coordinates to a comma-separated values file (.csv) that has the same name as the first image, but ends on '-landmarks.csv'. This file has one header row, and subsequently one row per landmark  and can be easily openend in e.g. excel or using pandas.", width=600)
        msg.pack()
